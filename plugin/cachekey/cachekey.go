...
func Generate(tmpl, mount string, data metadata.Metadata) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("parse, <%s> as cache key template, falling back to default %v", tmpl, err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("build, <%s> as cache key, falling back to default %v", tmpl, err)
	}
...
func readerHasher(readers ...io.Reader) (string, error) {
	...
	for _, r := range readers {
		if _, err := io.Copy(h, r); err != nil {
			return "", fmt.Errorf("write reader as hash %v", err)
		}
	}
